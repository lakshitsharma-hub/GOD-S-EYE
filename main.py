...
def check_market_signals():
    ...
    try:
        # Fetch OHLCV candles (Returns raw list of lists instead of pandas DataFrame)
        bars = exchange.fetch_ohlcv(SYMBOL, TIMEFRAME, limit=100)
        length = len(bars)
        if length < 20:
            return

        # bars[i] structure: [timestamp, open, high, low, close, volume]
        idx = length - 1 - RIGHT_BARS
        if idx < LEFT_BARS:
            return
            
        current_candle_time = bars[idx][0]
        pivot_high_val = bars[idx][2] # High
        pivot_low_val = bars[idx][3]  # Low
