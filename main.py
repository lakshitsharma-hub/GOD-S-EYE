...
# Toggle: set ENABLE_OWN_TRADING=true on Render to re-enable Render's own
# Binance scanning/trading. Default is OFF — Render now acts purely as a
# relay/notification hub for the HuggingFace bot's trades.
ENABLE_OWN_TRADING = os.environ.get("ENABLE_OWN_TRADING", "false").lower() == "true"

if __name__ == "__main__":
    # Start Flask to keep Render Web Service alive (also serves /webhook/relay)
    threading.Thread(target=run_flask, daemon=True).start()
    
    # Start the robust Telemetry Message Queue Worker (handles relay messages too)
    threading.Thread(target=telemetry_worker, daemon=True).start()
    
    time.sleep(3)
    
    if ENABLE_OWN_TRADING:
        # Start the actual scanning engine
        run_bot()
    else:
        print("🛰️ Own trading scan DISABLED — running as relay-only. Set ENABLE_OWN_TRADING=true to re-enable.", flush=True)
        # Keep the main thread alive so the daemon threads (Flask, telemetry) keep running
        while True:
            time.sleep(3600)
